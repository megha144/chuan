...
class PeoplePoseArray {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.poses.forEach((val) => {
      bufferOffset = PeoplePose.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new PeoplePoseArray(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.poses[i] = PeoplePose.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.poses.forEach((val) => {
      length += PeoplePose.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new PeoplePoseArray(null);
    ...
    if (msg.poses !== undefined) {
      ...
      for (let i = 0; i < resolved.poses.length; ++i) {
        resolved.poses[i] = PeoplePose.Resolve(msg.poses[i]);
      }
    }
    else {
      resolved.poses = []
...
module.exports = PeoplePoseArray;
