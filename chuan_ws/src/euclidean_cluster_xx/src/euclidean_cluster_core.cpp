...
void EuClusterCore::cluster_by_distance(pcl::PointCloud<pcl::PointXYZ>::Ptr in_pc, std::vector<Detected_Obj> &obj_list)
{
    ...
    for (size_t i = 0; i < in_pc->points.size(); i++)
    {
        ...
        // 如果点的距离大于120m, 忽略该点
        //限定在前半径 50m  的园内
        if (origin_distance >= 5 || current_point.x < 0 ||current_point.z < -0.2)   
        {
            continue;
        }


        if (origin_distance < seg_distance_[0])  //15m
        {
            segment_pc_array[0]->points.push_back(current_point);
        }
        else if (origin_distance < seg_distance_[1])  //30m
        {
            segment_pc_array[1]->points.push_back(current_point);
        }
        else if (origin_distance < seg_distance_[2])   //45m
        {
            segment_pc_array[2]->points.push_back(current_point);
        }
        else if (origin_distance < seg_distance_[3])  //60m
        {
    ...
    for (size_t i = 0; i < segment_pc_array.size(); i++)
    {
      //15m 30m 45m 60m 其他 处分别用半径 0.5, 1.0, 1.5, 2.0, 2.5 作聚类分割提取
...
//xx
void EuClusterCore::flag_nav_to_3dlidar_cb(const xx_msgs::Flag::ConstPtr& msg)
{
    std::string flag_nav;
    flag_nav = msg->flag;
    std::cout<< flag_nav << std::endl;
    if(flag_nav == "nav stop,3dlidar start")  //来自nav
    {
        flag_3dlidar_stauts = 1;
    }
    if(flag_nav == "3dbox need stop")  //来自 3dbox
    {
        flag_3dlidar_stauts = 0;
    }
}